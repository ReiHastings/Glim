...
//   zones). Before anything executes, two pre-flight checks run against the
//   real manifest and fail the whole run on any mismatch:
//     coverage - every tests/*.test.mjs on disk has a row, every row's file exists
//     fidelity - each file's header `usage:` line agrees with its rows (hook, zones)
...
export const MANIFEST = [
  ...
  // Bare node (static checks and pure arithmetic)
...
// --- Pre-flight: coverage and fidelity --------------------------------------
...
// Reads the header (first 80 lines) and returns { hooks:Set, zones:Set } as the
// file's own usage lines declare them. An absent TZ on a usage line is recorded
// as null, meaning "no pin declared".
async function declaredUsage(root, file) {
  ...
  for (const m of head.matchAll(USAGE_RE)) {
...
export async function preflight(root, manifest) {
  ...
  // Step 1: coverage, both directions.
  ...
  // Step 2: fidelity against each file's own usage lines.
  for (const f of inManifest) {
    if (!onDisk.includes(f)) continue;
    const rows = manifest.filter((r) => r.file === f);
    const { hooks, zones } = await declaredUsage(root, f);
    if (hooks.size === 0) { problems.push(`fidelity: ${f} has no parsable usage line in its header`); continue; }
    ...
    if (declared.length > 0) {
      // Header pins zones: the manifest must run exactly that set.
...
export function runRow(r, { root, timeoutMs }) {
  return new Promise((resolve) => {
    ...
    // 'close' fires after both stdio streams have ended, so the captured
    // output is complete at this point (the self-test checks this).
    child.on('close', (code, signal) => {
      clearTimeout(timer);
      const status = timedOut ? 'TIMEOUT' : code === 0 ? 'PASS' : 'FAIL';
      resolve({ row: r, status, code, signal, ms: Date.now() - started, output: Buffer.concat(chunks).toString('utf8') });
    });
    child.on('error', (err) => {
      clearTimeout(timer);
      resolve({ row: r, status: 'FAIL', code: null, signal: null, ms: Date.now() - started, output: String(err) });
    });
  });
}
...
export async function runAll(root, manifest, opts = {}) {
  ...
  if (selected.length === 0) {
    log(`no rows selected${only ? ` for --only ${only}` : ''}`);
  ...
  log(`\n${manifest.length} rows: ${results.length - failures.length} passed, ${failures.length} failed, ${skipped} skipped` +
      (only ? ` (${manifest.length - selected.length - skipped} not selected)` : ''));
...
async function selfTest() {
  ...
  await mkdir(path.join(tmp, 'tests'));
  const write = (name, body) => writeFile(path.join(tmp, 'tests', name), body);
  const header = (name, hookFlag = '', tz = '') =>
    `// usage:\n//   cd client && ${tz ? `TZ=${tz} ` : ''}node ${hookFlag}tests/${name}\n`;
  ...
  try {
    ...
    check('1 failing script fails the run and is named', !r.ok && q.lines.some((l) => l.includes('FAIL') && l.includes('bad')));

    // 2. Coverage fires: file on disk, no row.
    await write('orphan.test.mjs', header('orphan.test.mjs') + 'process.exit(0);\n');
    q = quiet();
    r = await runAll(tmp, [row('bad.test.mjs', 'none')], { log: q.log });
    check('2 uncovered file fails pre-flight, nothing runs',
      !r.ok && r.results.length === 0 && q.lines.some((l) => l.includes('coverage') && l.includes('orphan')));
    await rm(path.join(tmp, 'tests', 'orphan.test.mjs'));

    // 3. Fidelity fires: header says hooks, manifest says none.
    await write('mismatch.test.mjs', header('mismatch.test.mjs', '--import ./tests/register-hooks.mjs ') + 'process.exit(0);\n');
    q = quiet();
    r = await runAll(tmp, [row('bad.test.mjs', 'none'), row('mismatch.test.mjs', 'none')], { log: q.log });
    check('3 header/manifest hook mismatch fails pre-flight',
      !r.ok && r.results.length === 0 && q.lines.some((l) => l.includes('fidelity') && l.includes('mismatch') && l.includes('hooks')));
    await rm(path.join(tmp, 'tests', 'mismatch.test.mjs'));

    // 4. TZ reaches the child.
    await write('tz.test.mjs', header('tz.test.mjs', '', 'Australia/Lord_Howe') + 'console.log("TZ=" + process.env.TZ);\n');
    const res = await runRow(row('tz.test.mjs', 'none', 'Australia/Lord_Howe'), { root: tmp, timeoutMs: 10_000 });
    check('4 TZ from the row reaches the child', res.status === 'PASS' && res.output.includes('TZ=Australia/Lord_Howe'), res.output);
    ...
    // 6. Non-hang: a child that never exits is killed and reported.
    ...
    check('6 hung child is killed and reported TIMEOUT within 5 s', hang.status === 'TIMEOUT' && Date.now() - t0 < 5_000);

    // Clear the tree so checks 7 and 8 see exactly one covered file.
    await write('ok.test.mjs', header('ok.test.mjs') + 'process.exit(0);\n');
    await rm(path.join(tmp, 'tests', 'bad.test.mjs'));
    await rm(path.join(tmp, 'tests', 'big.test.mjs'));
    await rm(path.join(tmp, 'tests', 'hang.test.mjs'));
    await rm(path.join(tmp, 'tests', 'tz.test.mjs'));

    // 7. Empty selection is a failure (pre-flight passes, nothing matches).
    q = quiet();
    r = await runAll(tmp, [row('ok.test.mjs', 'none')], { only: 'nonexistent', log: q.log });
    check('7 --only matching nothing fails', !r.ok && r.problems.includes('empty selection'));

    // 8. --fast skips slow rows and counts them.
    q = quiet();
    r = await runAll(tmp, [row('ok.test.mjs', 'none', NY, true)], { fast: true, log: q.log });
    check('8 --fast skips slow rows and reports them skipped', !r.ok && r.skipped === 1 && r.results.length === 0);
    q = quiet();
    r = await runAll(tmp, [row('ok.test.mjs', 'none', NY, true)], { log: q.log });
    check('8b the same slow row runs without --fast', r.ok && r.results.length === 1);
  } finally {
...
async function main(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--only') { opts.only = argv[++i]; if (!opts.only) { usage(); return 2; } }
